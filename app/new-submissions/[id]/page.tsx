...
export default function NewSubmissionReviewPage() {
  ...
  useEffect(() => {
    ...
    const fetchSubmission = async () => {
      ...
      if (error || !data) {
        ...
        router.replace('/new-submissions')
        return
      }
  ...
  // Yêu cầu SenAI giải thích chi tiết câu hỏi
        ...
        <div className="flex items-center gap-3">
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-center font-black">
            ...
            <span className="text-sm sm:text-base text-amber-600 dark:text-amber-400 font-black">
              {submission.score} <span className="text-xs font-semibold opacity-75">/ 10</span>
            </span>
          </div>
                          ...
                          {aiExplains[key] ? (
                            <div className="rounded-xl border border-indigo-500/30 bg-indigo-500/5 p-3.5 text-xs leading-relaxed space-y-2">
                              <span className="font-black text-indigo-600 dark:text-indigo-400 flex items-center gap-1.5">
                                <Sparkles className="h-3.5 w-3.5" /> Giải thích từ SenAI:
                              </span>
                            ...
                            >
                              {aiLoadingKey === key ? (
                                <>
                                  <Loader2 className="h-3.5 w-3.5 animate-spin" /> SenAI đang phân tích câu hỏi...
                                </>
                              ) : (
                                <>
