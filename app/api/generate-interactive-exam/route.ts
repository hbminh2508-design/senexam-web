...
4b. Nhận diện HÌNH ẢNH: nếu câu hỏi có nhắc tới một hình vẽ/đồ thị/bảng số liệu không nằm trong phần text (ví dụ "cho hình bên dưới", "quan sát hình vẽ", "cho đồ thị", "dựa vào bảng số liệu sau"), bạn sẽ được cung cấp thêm ảnh chụp từng trang của đề bài (đánh số theo thứ tự "Trang 1", "Trang 2", ...). Hãy xác định hình đó nằm ở trang nào và toạ độ khung bao quanh NÓ (chỉ hình vẽ, KHÔNG bao gồm phần chữ câu hỏi), rồi thêm trường questionEntries[qIdx].imageRef = { "page": <số trang, bắt đầu từ 1>, "box": [ymin, xmin, ymax, xmax] } với box là toạ độ chuẩn hoá theo thang 0-1000 trên trang ảnh đó (0,0 là góc trên-trái, 1000,1000 là góc dưới-phải). Nếu câu không có hình, KHÔNG thêm trường imageRef.
  ...
  "sections": [
    {
      ...
      "questionCount": 10,
      "questionEntries": { "0": { "text": "...", "options": ["...", "...", "...", "..."], "imageRef": { "page": 2, "box": [120, 80, 430, 620] } } },
      "correctAnswers": { "0": "A" }
    }
  ]
}`
...
export async function POST(request: Request) {
  try {
    const { files, pageImages } = await request.json() as {
    ...
    type ParsedSection = {
      ...
      questionEntries?: Record<string, { text: string; options?: string[]; imageRef?: { page: number; box: [number, number, number, number] } }>
